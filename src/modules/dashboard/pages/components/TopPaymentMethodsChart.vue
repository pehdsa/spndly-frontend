...
import type { TopPaymentMethod } from '@/services/dashboard'
...
const x = (_: TopPaymentMethod, i: number) => i

const yAccessors = computed(() =>
  chartData.value.map((item, idx) => (_d: TopPaymentMethod, i: number) => i === idx ? item.total_amount / 100 : 0),
)

const tickFormat = (i: number) => chartData.value[i]?.name ?? ''

const tooltipTemplate = (barDatum: { datum: TopPaymentMethod }) => {
...
<template>
  <Card>
    <CardHeader>
      <CardTitle class="text-base">Top Métodos de Pagamento</CardTitle>
    </CardHeader>
